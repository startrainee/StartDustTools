package com.stardust.tools.startdusttools.widgets.swiperefreshlayout;
 ...
 * 可以设置setPushDisabled(boolean isAble),setPullDisabled(boolean isAble) 停用下拉和上拉;
 */
...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    public SuperSwipeRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mHeaderViewWidth = (int) (display != null ? display.getWidth() : 0);
        mFooterViewWidth = (int) (display != null ? display.getWidth() : 0);
     ...
     * 设置下拉是否可用
     * */
    public void setPushDisabled(boolean canPush){
        mCanPush = canPush;
    }
    /**
     * 设置上拉是否可用
     * */
    public void setPullDisabled(boolean canPull){
        mCanPull = canPull;
    }
    /**
     * 添加底部布局
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isChildScrollToBottom()) {
                    ...
                    if (yDiff > mTouchSlop && !mIsBeingDragged) {// 判断是否下拉的距离足够
                        mIsBeingDragged = mCanPull;// 正在上拉
                    }
                } else {
                    yDiff = y - mInitialMotionY;// 计算下拉距离
                    if (yDiff > mTouchSlop && !mIsBeingDragged) {// 判断是否下拉的距离足够
                        mIsBeingDragged = mCanPush;// 正在下拉
                    }
                }
                break;
